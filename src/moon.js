 ...
 */
M.physical = function (jde, earth) { // (jde float64, earth *pp.V87Planet)  (l, b, P, l0, b0 float64)
 ...
 */
M.sunAltitude = function (η, θ, l0, b0) { // (η, θ, l0, b0 float64)  float64
  let c0 = Math.PI / 2 - l0
 ...
 */
M.sunrise = function (η, θ, jde, earth) { // (η, θ, jde float64, earth *pp.V87Planet)  float64
 ...
 */
M.sunset = function (η, θ, jde, earth) { // (η, θ, jde float64, earth *pp.V87Planet)  float64
  jde += srCorr(η, θ, jde, earth)
  return jde + srCorr(η, θ, jde, earth)
}

function srCorr (η, θ, jde, earth) { // (η, θ, jde float64, earth *pp.V87Planet)  float64
  let phy = M.physical(jde, earth)
  let h = M.sunAltitude(η, θ, phy[3], phy[4]) // l0, b0
