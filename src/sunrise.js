...
class Sunrise {
  ...
  _calc (h0, isSet) {
    ...
    try {
      ...
      if ( // overlap with march, september equinoxes
        this.lat > 0 && (doy > 79 && doy < 267) ||  // northern hemisphere
        this.lat < 0 && (doy < 83 || doy > 263)     // southern hemisphere
      ) {
        step = -step
      }
  ...
  /**
   * @return {julian.Calendar} time of start of golden hour (starts before sunset start)
   */
  goldenHourStart () {
  ...
  /**
   * @return {julian.Calendar} time of end of golden hour (ends after sunrise end)
   */
  goldenHourEnd () {
