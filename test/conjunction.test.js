...
describe('#conjunction', function () {
  it('planetary', function () {
    ...
    var err
    try {
      var a = conjunction.planetary(day1, day5, r1, d1, r2, d2)
      var day = a[0]
      var Δδ = a[1]
    } catch (e) {
      err = e
  ...
  it('stellar', function () {
    ...
    var r1 = new sexa.RA(15, 17, 0.421 + pmr * dc).rad()
    ...
    var d1 = new sexa.Angle(true, 9, 22, 58.54 - pmd * dc).rad()

    assert.equal(new sexa.RA(r1).toString(3), '15ʰ17ᵐ.446ˢ')      // α′ = 15ʰ17ᵐ0ˢ.446
    assert.equal(new sexa.Angle(d1).toString(2), '-9°22′58.47″')  // δ′ = -9°22′58″.47

    var err
    try {
      var a = conjunction.stellar(day1, day5, r1, d1, r2, d2)
    } catch (e) {
      err = e
    }
    assert.ok(!err, '' + err)
