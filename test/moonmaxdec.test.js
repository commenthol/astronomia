...
describe('moonmaxdec', function () {
  it('north', function () {
    ...
    assert.equal((gd * 180 / Math.PI).toFixed(4), 28.1562)
    assert.equal(new sexa.Angle(gd).toString(0), '28°9′22″')
  })
  ...
  it('south', function () {
    ...
    assert.equal((gd * 180 / Math.PI).toFixed(4), -22.1384)
    assert.equal(new sexa.Angle(gd).toString(0), '-22°8′18″')
  })
  ...
  it('north c', function () {
    ...
    assert.equal((gd * 180 / Math.PI).toFixed(4), 28.9739)
    assert.equal(new sexa.Angle(gd).toString(0), '28°58′26″')
  })
})
