...
M.sep = function (c1, c2) {
  ...
  var cd = sd1 * sd2 + cd1 * cd2 * Math.cos(c1.ra - c2.ra); // (17.1) p. 109
...
M.minSepRect = function (jd1, jd3, cs1, cs2) {
  ...
  var uv = function uv(c1, c2) {
    ...
    var sΔd = Math.sin(c2.dec - c1.dec);
...
M.sepHav = function (c1, c2) {
  // using (17.5) p. 115
  return 2 * Math.asin(Math.sqrt(M.hav(c2.dec - c1.dec) + Math.cos(c1.dec) * Math.cos(c2.dec) * M.hav(c2.ra - c1.ra)));
};
...
M.sepPauwels = function (c1, c2) {
  ...
  var cdr = Math.cos(c2.ra - c1.ra);
  var x = cd1 * sd2 - sd1 * cd2 * cdr;
  var y = cd2 * Math.sin(c2.ra - c1.ra);
...
M.relativePosition = function (c1, c2) {
  ...
  var p = Math.atan2(sΔr, cd2 * Math.tan(c1.dec) - sd2 * cΔr);
  return p;
};