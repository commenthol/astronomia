...
M.minSepRect = function (jd1, jd3, cs1, cs2) {
  ...
  var uv = function uv(c1, c2) {
    ...
    var gDr = c2.ra - c1.ra;
    var tangDr = tan(gDr);
    var tanhgDr = tan(gDr / 2);
    var K = 1 / (1 + sind1 * sind1 * tangDr * tanhgDr);
    var singDd = sin(c2.dec - c1.dec);
    var u = -K * (1 - sind1 / cosd1 * singDd) * cosd1 * tangDr;
    var v = K * (singDd + sind1 * cosd1 * tangDr * tanhgDr);
    return [u, v];
  };
...
M.relativePosition = function (c1, c2) {
  var _base$sincos11 = base.sincos(c2.ra - c1.ra),
      ...
      singDr = _base$sincos12[0],
      cosgDr = _base$sincos12[1];
  ...
  var p = atan2(singDr, cosd2 * tan(c1.dec) - sind2 * cosgDr);
  return p;
};