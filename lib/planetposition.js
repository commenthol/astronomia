...
var Planet = function () {
  ...
  _createClass(Planet, [{
    ...
    value: function position2000(jde) {
      ...
      var gt = T * 0.1;
      var lon = base.pmod(sum(gt, this.series.L), 2 * Math.PI);
      var lat = sum(gt, this.series.B);
      var range = sum(gt, this.series.R);
...
M.toFK5 = function (lon, lat, jde) {
  // formula 32.3, p. 219.
  var T = base.J2000Century(jde);
  var Lp = lon - 1.397 * Math.PI / 180 * T - 0.00031 * Math.PI / 180 * T * T;
  ...
  var L5 = lon + -0.09033 / 3600 * Math.PI / 180 + 0.03916 / 3600 * Math.PI / 180 * (cLp + sLp) * Math.tan(lat);
  var B5 = lat + 0.03916 / 3600 * Math.PI / 180 * (cLp - sLp);
