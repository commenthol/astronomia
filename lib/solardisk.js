...
M.ephemeris = function (jd, earth) {
  ...
  var gth = (jd - 2398220) * 2 * Math.PI / 25.38;
  ...
  var ge = ge0 + gDge;
  var gl = L - 20.4898 / 3600 * Math.PI / 180 / R;
  var glp = gl + gDgps;

  var _base$sincos = base.sincos(gl - K),
      _base$sincos2 = _slicedToArray(_base$sincos, 2),
      sglK = _base$sincos2[0],
      cglK = _base$sincos2[1];
  ...
  var tx = -Math.cos(glp) * Math.tan(ge);
  var ty = -cglK * Math.tan(I);
  var P = Math.atan(tx) + Math.atan(ty);
  var B0 = Math.asin(sglK * sI);
  var gh = Math.atan2(-sglK * cI, -cglK);
