...
var Ecliptic = function () {
  ...
  _createClass(Ecliptic, [{
    key: 'toEquatorial',
    value: function toEquatorial(ge) {
      var _base$sincos = base.sincos(ge),
          _base$sincos2 = _slicedToArray(_base$sincos, 2),
          gesin = _base$sincos2[0],
          gecos = _base$sincos2[1];
      ...
      var ra = Math.atan2(sgl * gecos - sgb / cgb * gesin, cgl); // (13.3) p. 93
      if (ra < 0) {
        ra += 2 * Math.PI;
      }
      var dec = Math.asin(sgb * gecos + cgb * gesin * sgl); // (13.4) p. 93
...
var Equatorial = function () {
  ...
  _createClass(Equatorial, [{
    key: 'toEcliptic',
    value: function toEcliptic(ge) {
      var _base$sincos7 = base.sincos(ge),
          _base$sincos8 = _slicedToArray(_base$sincos7, 2),
          gesin = _base$sincos8[0],
          gecos = _base$sincos8[1];

      var _base$sincos9 = base.sincos(this.ra),
          _base$sincos10 = _slicedToArray(_base$sincos9, 2),
          sga = _base$sincos10[0],
          cga = _base$sincos10[1];
      ...
      var lon = Math.atan2(sga * gecos + sgd / cgd * gesin, cga); // (13.1) p. 93
      var lat = Math.asin(sgd * gecos - cgd * gesin * sga); // (13.2) p. 93
    ...
    value: function toHorizontal(g, st) {
      ...
      var azimuth = Math.atan2(sH, cH * sgf - sgd / cgd * cgf); // (13.5) p. 93
      var altitude = Math.asin(sgf * sgd + cgf * cgd * cH); // (13.6) p. 93
  ...
  _createClass(Horizontal, [{
    ...
    value: function toEquatorial(g, st) {
      ...
      var _base$sincos29 = base.sincos(g.lat),
          ...
          sgf = _base$sincos30[0],
          cgf = _base$sincos30[1];

      var H = Math.atan2(sA, cA * sgf + sh / ch * cgf);
      var ra = base.pmod(new sexa.Time(st).rad() - g.lon - H, 2 * Math.PI);
      var dec = Math.asin(sgf * sh - cgf * ch * cA);
      return new Equatorial(ra, dec);
    }
  }]);
  ...
  _createClass(Galactic, [{
    ...
    value: function toEquatorial() {
      ...
      var _base$sincos33 = base.sincos(galacticNorth1950.dec),
          ...
          sggd = _base$sincos34[0],
          cggd = _base$sincos34[1];
