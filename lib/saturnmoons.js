...
M.positions = function (jde, earth, saturn) {
  ...
  f();
  f();
  var gl0 = Math.atan2(y, x);
  var gb0 = Math.atan(z / Math.hypot(x, y));
  var ecl = new coord.Ecliptic(gl0, gb0);
  ...
  for (j = 1; j <= 8; j++) {
    var u = s4[j].gl - s4[j].gw;
    var w = s4[j].gw - 168.8112 * d;
    ...
    var _base$sincos11 = base.sincos(s4[j].gg),
        _base$sincos12 = _slicedToArray(_base$sincos11, 2),
        sgg = _base$sincos12[0],
        cgg = _base$sincos12[1];
  ...
  var _base$sincos13 = base.sincos(gl0),
      _base$sincos14 = _slicedToArray(_base$sincos13, 2),
      sgl0 = _base$sincos14[0],
      cgl0 = _base$sincos14[1];

  var _base$sincos15 = base.sincos(gb0),
      _base$sincos16 = _slicedToArray(_base$sincos15, 2),
      sgb0 = _base$sincos16[0],
      cgb0 = _base$sincos16[1];
...
Qs.prototype.tethys = function () {
  var r = new R4();
  r.gl = 285.306 * d + 190.69791226 * d * this.t1 + 2.063 * d * this.sW0 + 0.03409 * d * this.s3W0 + 0.001015 * d * this.s5W0;
  r.r = 4.880998;
  r.gg = 1.0976 * d;
...
Qs.prototype.rhea = function () {
  ...
  var glʹ = 359.244 * d + 79.6900472 * d * this.t1 + 0.086754 * d * sN;
  var i = 28.0362 * d + 0.346898 * d * cN + 0.0193 * d * this.cW3;
  var gw = 168.8034 * d + 0.736936 * d * sN + 0.041 * d * this.sW3;
  var a = 8.725924;
...
Qs.prototype.subr = function (glʹ, p, e, a, gw, i) {
  ...
  var gps = Math.atan2(this.s1 * sg, h);
...
Qs.prototype.titan = function () {
  ...
  var gwʹ = 168.66925 * d + 0.628808 * d * this.sW3;
  ...
  var _base$sincos29 = base.sincos(gwʹ - this.W8),
      _base$sincos30 = _slicedToArray(_base$sincos29, 2),
      sgwʹW8 = _base$sincos30[0],
      cgwʹW8 = _base$sincos30[1];

  var a1 = this.sW7 * sgwʹW8;
  var a2 = this.cW7 * siʹ - this.sW7 * ciʹ * cgwʹW8;
  var g0 = 102.8623 * d;
  var gps = Math.atan2(a1, a2);
  var s = Math.hypot(a1, a2);
  var g = this.W4 - gwʹ - gps;
  var ϖ = 0;
  ...
  var qq = 2 * (this.W5 - ϖ);
  var b1 = siʹ * sgwʹW8;
  var b2 = this.cW7 * siʹ * cgwʹW8 - this.sW7 * ciʹ;
  var gth = Math.atan2(b1, b2) + this.W8;
  ...
  var h = 0.9375 * eʹ * eʹ * sq + 0.1875 * s * s * Math.sin(2 * (this.W5 - gth));
  var glʹ = L - 0.254744 * d * (this.e1 * Math.sin(this.W6) + 0.75 * this.e1 * this.e1 * Math.sin(2 * this.W6) + h);
  var i = iʹ + 0.031843 * d * s * cu;
  var gw = gwʹ + 0.031843 * d * s * su / siʹ;
  var a = 20.216193;
  return this.subr(glʹ, p, e, a, gw, i);
};

Qs.prototype.hyperion = function () {
  var gh = 92.39 * d + 0.5621071 * d * this.t6;
  var gz = 148.19 * d - 19.18 * d * this.t8;
  var gth = 184.8 * d - 35.41 * d * this.t9;
  var gthʹ = gth - 7.5 * d;
  ...
  var gf = 2 * (ϖ - this.W5);
  var gx = 94.9 * d - 2.292 * d * this.t8;

  var _base$sincos37 = base.sincos(gh),
      _base$sincos38 = _slicedToArray(_base$sincos37, 2),
      sgh = _base$sincos38[0],
      cgh = _base$sincos38[1];

  var _base$sincos39 = base.sincos(gz),
      _base$sincos40 = _slicedToArray(_base$sincos39, 2),
      sgz = _base$sincos40[0],
      cgz = _base$sincos40[1];

  var _base$sincos41 = base.sincos(2 * gz),
      _base$sincos42 = _slicedToArray(_base$sincos41, 2),
      s2gz = _base$sincos42[0],
      c2gz = _base$sincos42[1];

  var _base$sincos43 = base.sincos(3 * gz),
      _base$sincos44 = _slicedToArray(_base$sincos43, 2),
      s3gz = _base$sincos44[0],
      c3gz = _base$sincos44[1];

  var _base$sincos45 = base.sincos(gz + gh),
      _base$sincos46 = _slicedToArray(_base$sincos45, 2),
      sgzpgh = _base$sincos46[0],
      cgzpgh = _base$sincos46[1];

  var _base$sincos47 = base.sincos(gz - gh),
      _base$sincos48 = _slicedToArray(_base$sincos47, 2),
      sgzmgh = _base$sincos48[0],
      cgzmgh = _base$sincos48[1];

  var _base$sincos49 = base.sincos(gf),
      _base$sincos50 = _slicedToArray(_base$sincos49, 2),
      sgf = _base$sincos50[0],
      cgf = _base$sincos50[1];

  var _base$sincos51 = base.sincos(gx),
      _base$sincos52 = _slicedToArray(_base$sincos51, 2),
      sgx = _base$sincos52[0],
      cgx = _base$sincos52[1];
  ...
  var a = 24.50601 - 0.08686 * cgh - 0.00166 * cgzpgh + 0.00175 * cgzmgh;
  var e = 0.103458 - 0.004099 * cgh - 0.000167 * cgzpgh + 0.000235 * cgzmgh + 0.02303 * cgz - 0.00212 * c2gz + 0.000151 * c3gz + 0.00013 * cgf;
  var p = ϖ + 0.15648 * d * sgx - 0.4457 * d * sgh - 0.2657 * d * sgzpgh - 0.3573 * d * sgzmgh - 12.872 * d * sgz + 1.668 * d * s2gz - 0.2419 * d * s3gz - 0.07 * d * sgf;
  var glʹ = 177.047 * d + 16.91993829 * d * this.t6 + 0.15648 * d * sgx + 9.142 * d * sgh + 0.007 * d * Math.sin(2 * gh) - 0.014 * d * Math.sin(3 * gh) + 0.2275 * d * sgzpgh + 0.2112 * d * sgzmgh - 0.26 * d * sgz - 0.0098 * d * s2gz - 0.013 * d * Math.sin(as) + 0.017 * d * Math.sin(bs) - 0.0303 * d * sgf;
  var i = 27.3347 * d + 0.6434886 * d * cgx + 0.315 * d * this.cW3 + 0.018 * d * Math.cos(gth) - 0.018 * d * ccs;
  var gw = 168.6812 * d + 1.40136 * d * cgx + 0.68599 * d * this.sW3 - 0.0392 * d * scs + 0.0366 * d * Math.sin(gthʹ);
  return this.subr(glʹ, p, e, a, gw, i);
};
...
Qs.prototype.iapetus = function () {
  ...
  var gps = 4.367 * d - 0.195 * d * this.t7;
  var gth = 146.819 * d - 3.198 * d * this.t7;
  var gf = 60.47 * d + 1.521 * d * this.t7;
  var gF = 205.055 * d - 2.091 * d * this.t7;
  var eʹ = 0.028298 + 0.001156 * this.t11;
  var ϖ0 = 352.91 * d + 11.71 * d * this.t11;
  var gm = 76.3852 * d + 4.53795125 * d * this.t10;
  var iʹ = base.horner(this.t11, 18.4602 * d, -0.9518 * d, -0.072 * d, 0.0054 * d);
  var gwʹ = base.horner(this.t11, 143.198 * d, -3.919 * d, 0.116 * d, 0.008 * d);
  var l = gm - ϖ0;
  var g = ϖ0 - gwʹ - gps;
  var g1 = ϖ0 - gwʹ - gf;
  var ls = this.W5 - ϖʹ;
  ...
  var _base$sincos71 = base.sincos(u5 + gps),
      _base$sincos72 = _slicedToArray(_base$sincos71, 2),
      su5gps = _base$sincos72[0],
      cu5gps = _base$sincos72[1];

  var _base$sincos73 = base.sincos(u2 + gf),
      _base$sincos74 = _slicedToArray(_base$sincos73, 2),
      su2gf = _base$sincos74[0],
      cu2gf = _base$sincos74[1];

  var _base$sincos75 = base.sincos(l + g1 + lT + gT + gf),
  ...
  var p = ϖ0 + w / eʹ;
  var glʹ = gm - 0.04299 * d * su2 - 0.00789 * d * su1 - 0.06312 * d * Math.sin(ls) - 0.00295 * d * Math.sin(2 * ls) - 0.02231 * d * Math.sin(u5) + 0.0065 * d * su5gps;
  var i = iʹ + 0.04204 * d * cu5gps + 0.00235 * d * c5 + 0.0036 * d * cu2gf;
  var wʹ = 0.04204 * d * su5gps + 0.00235 * d * s5 + 0.00358 * d * su2gf;
  var gw = gwʹ + wʹ / Math.sin(iʹ);
  return this.subr(glʹ, p, e, a, gw, i);
};