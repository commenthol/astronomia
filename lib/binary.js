...
M.position = function (a, e, i, Ω, ω, E) {
  ...
  var sνω = _base$sincos2[0];
  var cνω = _base$sincos2[1];

  var ci = Math.cos(i);
  var num = sνω * ci;
  var θ = Math.atan2(num, cνω) + Ω;
  if (θ < 0) {
    θ += 2 * Math.PI;
  }
  var ρ = r * Math.sqrt(num * num + cνω * cνω);
  return [θ, ρ];
};
...
M.apparentEccentricity = function (e, i, ω) {
  ...
  var A = (1 - e * e * cω * cω) * ci * ci;
  var B = e * e * sω * cω * ci;
  var C = 1 - e * e * sω * sω;
  var d = A - C;
  var sD = Math.sqrt(d * d + 4 * B * B);
