...
M.position = function (planet, earth, jde) {
  // (p, earth *pp.V87Planet, jde float64)  (ga, gd float64)
  var x = void 0;
  var y = void 0;
  var z = void 0;
  ...
  function pos() {
    var gt = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;

    var pos = planet.position(jde - gt);
  ...
  pos();
  var gD = Math.sqrt(x * x + y * y + z * z); // (33.4) p. 224
  var gt = base.lightTime(gD);
  // repeating with jde-gt
  pos(gt);

  var gl = Math.atan2(y, x); // (33.1) p. 223
  var gb = Math.atan2(z, Math.hypot(x, y)); // (33.2) p. 223

  var _apparent$eclipticAbe = apparent.eclipticAberration(gl, gb, jde),
      _apparent$eclipticAbe2 = _slicedToArray(_apparent$eclipticAbe, 2),
      gDgl = _apparent$eclipticAbe2[0],
      gDgb = _apparent$eclipticAbe2[1];

  var fk5 = planetposition.toFK5(gl + gDgl, gb + gDgb, jde);
  gl = fk5.lon;
  gb = fk5.lat;
  ...
  _createClass(Elements, [{
    ...
    value: function position(jde, earth) {
      ...
      var _base$sincos9 = base.sincos(this.node),
          ...
          sgw = _base$sincos10[0],
          cgw = _base$sincos10[1];
      ...
      var H = sgw * sge;
      var P = -sgw * ci;
      var Q = cgw * ci * cge - si * sge;
      var R = cgw * ci * sge + si * cge;
      // (33.8) p. 229
      ...
      var f = function f(jde) {
        ...
        var gn = kepler.true(E, _this.ecc);
        var r = kepler.radius(E, _this.ecc, _this.axis);
        // (33.9) p. 229
        var x = r * a * Math.sin(A + _this.argP + gn);
        var y = r * b * Math.sin(B + _this.argP + gn);
        var z = r * c * Math.sin(C + _this.argP + gn);
        return { x: x, y: y, z: z };
      };
...
M.astrometricJ2000 = function (f, jde, earth) {
  // (f func(float64)  (x, y, z float64), jde float64, e *pp.V87Planet) (ga, gd, gps float64)
  ...
  var ga = Math.atan2(gh, gks);
  if (ga < 0) {
    ga += 2 * Math.PI;
  }
  var gd = Math.asin(gz / gD);
  var R0 = Math.sqrt(X * X + Y * Y + Z * Z);
  var gps = Math.acos((gks * X + gh * Y + gz * Z) / R0 / gD);
