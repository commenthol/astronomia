...
var g = function g(k, jm, c1, c2) {
  // (k, jm, c1, c2 float64)  (eclipse bool, jdeMax, gg, u, Mʹ float64)
  var ck = 1 / 1236.85;
  var p = Math.PI / 180;
  var T = k * ck;
  ...
  var gw = base.horner(T, 124.7746 * p, -1.56375588 * p / ck, 0.0020672 * p, 0.00000215 * p);
  var sgw = Math.sin(gw);
  var F1 = F - 0.02665 * p * sgw;
...
M.solar = function (year) {
  // (year float64)  (eclipseType int, central bool, jdeMax, gg, u, p, mag float64)
  ...
  var central = agg < 0.9972; // eclipse center touches Earth

  if (!central) {
    eclipseType = TYPE.Partial; // most common case
    if (agg < 1.026) {
      // umbral cone may touch earth
      if (agg < 0.9972 + Math.abs(u)) {
        // total or annular
    ...
    var gwa = 0.00464 * Math.sqrt(1 - gg * gg);
    if (u < gwa) {
...
M.lunar = function (year) {
  // (year float64)  (eclipseType int, jdeMax, gg, gr, gs, mag, sdTotal, sdPartial, sdPenumbral float64)
  var eclipseType = TYPE.None;
  var mag = void 0;
  var sdTotal = void 0;
