...
*/
M.nutation = function (ga, gd, jd) {
  // (ga, gd, jd float64)  (gDga1, gDgd1 float64)
  var ge = nutation.meanObliquity(jd);

  var _base$sincos = base.sincos(ge),
      _base$sincos2 = _slicedToArray(_base$sincos, 2),
      singe = _base$sincos2[0],
      cosge = _base$sincos2[1];
  ...
  var _base$sincos3 = base.sincos(ga),
      _base$sincos4 = _slicedToArray(_base$sincos3, 2),
      singa = _base$sincos4[0],
      cosga = _base$sincos4[1];

  var tangd = tan(gd);
  // (23.1) p. 151
  var gDga1 = (cosge + singe * singa * tangd) * gDgps - cosga * tangd * gDge;
  var gDgd1 = singe * cosga * gDgps + singa * gDge;
  return [gDga1, gDgd1];
};
 ...
 */
var gk = 20.49552 * Math.PI / 180 / 3600;
 ...
 */
M.eclipticAberration = function (gl, gb, jd) {
  // (gl, gb, jd float64)  (gDgl, gDgb float64)
  ...
  var _base$sincos7 = base.sincos(lon - gl),
      _base$sincos8 = _slicedToArray(_base$sincos7, 2),
      ssgl = _base$sincos8[0],
      csgl = _base$sincos8[1];

  var _base$sincos9 = base.sincos(gp - gl),
      _base$sincos10 = _slicedToArray(_base$sincos9, 2),
      singpgl = _base$sincos10[0],
      cosgpgl = _base$sincos10[1];
  // (23.2) p. 151


  var gDgl = gk * (e * cosgpgl - csgl) / cgb;
  var gDgb = -gk * sgb * (ssgl - e * singpgl);
  return [gDgl, gDgb];
};
 ...
 * coordinates of an object.
 */
M.aberration = function (ga, gd, jd) {
  // (ga, gd, jd float64)  (gDga2, gDgd2 float64)
  var ge = nutation.meanObliquity(jd);
  var T = base.J2000Century(jd);
  ...
  var _base$sincos17 = base.sincos(gp),
      _base$sincos18 = _slicedToArray(_base$sincos17, 2),
      singp = _base$sincos18[0],
      cosgp = _base$sincos18[1];

  var cosge = cos(ge);
  var q1 = cosga * cosge;
  // (23.3) p. 152
  var gDga2 = gk * (e * (q1 * cosgp + singa * singp) - (q1 * coss + singa * sins)) / cosgd;
  var q2 = cosge * (tan(ge) * cosgd - singa * singd);
  var q3 = cosga * singd;
  var gDgd2 = gk * (e * (cosgp * q2 + singp * q3) - (coss * q2 + sins * q3));
  return [gDga2, gDgd2];
};
...
M.position = function (eqFrom, epochFrom, epochTo, mga, mgd) {
  // (eqFrom, eqTo *coord.Equatorial, epochFrom, epochTo float64, mga sexa.HourAngle, mgd sexa.Angle)  *coord.Equatorial
  var eqTo = precess.position(eqFrom, epochFrom, epochTo, mga, mgd);
 ...
 */
M.aberrationRonVondrak = function (ga, gd, jd) {
  // (ga, gd, jd float64)  (gDga, gDgd float64)
  var T = base.J2000Century(jd);
  var r = {
    T: T,
  ...
  var _base$sincos19 = base.sincos(ga),
      _base$sincos20 = _slicedToArray(_base$sincos19, 2),
      singa = _base$sincos20[0],
      cosga = _base$sincos20[1];

  var _base$sincos21 = base.sincos(gd),
      _base$sincos22 = _slicedToArray(_base$sincos21, 2),
      singd = _base$sincos22[0],
      cosgd = _base$sincos22[1];
  // (23.4) p. 156
 ...
 */
M.positionRonVondrak = function (eqFrom, epochTo, mga, mgd) {
  // (eqFrom, eqTo *coord.Equatorial, epochTo float64, mga sexa.HourAngle, mgd sexa.Angle)  *coord.Equatorial
  var t = epochTo - 2000;
  ...
  var _M$aberrationRonVondr = M.aberrationRonVondrak(eqTo.ra, eqTo.dec, jd),
      ...
      gDga = _M$aberrationRonVondr2[0],
      gDgd = _M$aberrationRonVondr2[1];
  ...
  var _M$nutation3 = M.nutation(eqTo.ra, eqTo.dec, jd),
      ...
      gDga1 = _M$nutation4[0],
      gDgd1 = _M$nutation4[1];
