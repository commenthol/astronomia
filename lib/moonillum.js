...
function cosEq(ga, gd, ga0, gd0) {
  ...
  var _base$sincos3 = base.sincos(gd0),
      _base$sincos4 = _slicedToArray(_base$sincos3, 2),
      sgd0 = _base$sincos4[0],
      cgd0 = _base$sincos4[1];
...
function pa(gD, R, cgps) {
  var sgps = Math.sin(Math.acos(cgps));
  var i = Math.atan(R * sgps / (gD - R * cgps));
  if (i < 0) {
    i += Math.PI;
  }
  return i;
 ...
 * @private
 */
function cosEcl(gl, gb, gl0) {
  // (gl, gb, gl0 float64)  float64
