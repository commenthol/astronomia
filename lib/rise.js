...
// maintain backwards compatibility - will be removed in v2
function _compatibility(rs) {
  rs[0] = rs.rise; // for backward-compatibility
  rs[1] = rs.transit;
  rs[2] = rs.set;
  return rs;
}
...
M.times = function (p, ΔT, h0, Th0, α3, δ3) {
  ...
  var adjustRS = function adjustRS(m) {
    ...
    var _base$sincos9 = base.sincos(Hrad);
    ...
    var h = Math.asin(sLat * sδ + cLat * cδ * cH); // formula 13.6
    var Δm = SECS_PER_DAY * (h - h0) / (cδ * cLat * sH * 2 * Math.PI); // formula p103 3
    return m + Δm;
  };
