...
M.approxTimes = function (p, h0, Th0, α, δ) {
  // (p globe.Coord, h0, Th0 float64, α, δ float64)  (mRise, mTransit, mSet float64, err error)
  ...
  var cH0 = (Math.sin(h0) - sLat * sδ1) / (cLat * cδ1); // (15.1) p. 102
  if (cH0 < -1 || cH0 > 1) {
  ...
  // (15.2) p. 102.0
  var mt = (α + p.lon) * 43200 / Math.PI - Th0;
  var mTransit = base.pmod(mt, 86400);
  var mRise = base.pmod(mt - H0, 86400);
  var mSet = base.pmod(mt + H0, 86400);
...
M.times = function (p, ΔT, h0, Th0, α3, δ3) {
  ...
  var d3α = new interp.Len3(-86400, 86400, α3);
  var d3δ = new interp.Len3(-86400, 86400, δ3);

  // adjust mTransit
  var th0 = base.pmod(Th0 + mTransit * 360.985647 / 360, 86400);
  var α = d3α.interpolateX(mTransit + ΔT);
  var H = th0 - (p.lon + α) * 43200 / Math.PI;
  mTransit -= H;
  ...
  var adjustRS = function adjustRS(m) {
    // (float64, error) {
    var th0 = base.pmod(Th0 + m * 360.985647 / 360, 86400);
    var ut = m + ΔT;
    ...
    var h = sLat * sδ + cLat * cδ * Math.cos(H);
