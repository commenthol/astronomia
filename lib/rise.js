 ...
 */
M.stdh0Lunar = function (gp, refraction) {
...
M.hourAngle = function (lat, h0, gd) {
  ...
  var cosH = (sin(h0) - sin(lat) * sin(gd)) / (cos(lat) * cos(gd)); // (15.1) p. 102
  if (cosH < -1) {
...
function _mt(lon, ga, th0) {
  ...
  var mt = (lon + ga) * SECS_PER_DEGREE * 180 / Math.PI - th0;
  return mt;
}
...
M.approxTimes = function (p, h0, Th0, ga, gd) {
  var H0 = M.hourAngle(p.lat, h0, gd) * SECS_PER_DEGREE * 180 / Math.PI; // in degrees per day === seconds
  // approximate transit, rise, set times.
  // (15.2) p. 102.0
  var mt = _mt(p.lon, ga, Th0);
  var rs = {};
 ...
 * @throws Error
 */
M.times = function (p, gDT, h0, Th0, ga3, gd3) {
  // (p globe.Coord, gDT, h0, Th0 float64, ga3, gd3 []float64)  (mRise, mTransit, mSet float64, err error)
  var rs = M.approxTimes(p, h0, Th0, ga3[1], gd3[1]);
  var d3ga = new interp.Len3(-SECS_PER_DAY, SECS_PER_DAY, ga3);
  var d3gd = new interp.Len3(-SECS_PER_DAY, SECS_PER_DAY, gd3);

  // adjust mTransit
  var ut = rs.transit + gDT;
  var ga = d3ga.interpolateX(ut);
  var th0 = _th0(Th0, rs.transit);
  var H = -1 * _mt(p.lon, ga, th0); // in secs // Hmeus = 0...360
  rs.transit -= H;
  ...
  var adjustRS = function adjustRS(m) {
    var ut = m + gDT;
    var ga = d3ga.interpolateX(ut);
    var gd = d3gd.interpolateX(ut);
    var th0 = _th0(Th0, m);
    var H = -1 * _mt(p.lon, ga, th0);
    var Hrad = H / SECS_PER_DEGREE * D2R;
    var h = asin(sLat * sin(gd) + cLat * cos(gd) * cos(Hrad)); // formula 13.6
    var gDm = SECS_PER_DAY * (h - h0) / (cos(gd) * cLat * sin(Hrad) * 2 * Math.PI); // formula p103 3
    return m + gDm;
  };
...
var PlanetRise = function () {
  ...
  _createClass(PlanetRise, [{
    ...
    value: function times() {
      ...
      var Th0 = sidereal.apparent0UT(this.jd);
      var rs = M.times({ lat: this.lat, lon: this.lon }, this.gDT, this.refraction, Th0, this._toArr(body, 'ra'), this._toArr(body, 'dec'));
      return this._rsToJD(rs);
    }
    /** @private */
