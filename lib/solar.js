...
M.trueEquatorial = function (jde) {
  ...
  var ge = nutation.meanObliquity(jde);
  ...
  var _base$sincos3 = base.sincos(ge),
...
M.apparentEquatorial = function (jde) {
  ...
  var gl = M.apparentLongitude(T);
  var ge = nutation.meanObliquity(jde);

  var _base$sincos5 = base.sincos(gl),
  ...
  var _base$sincos7 = base.sincos(ge + 0.00256 * Math.PI / 180 * Math.cos(node(T))),
      _base$sincos8 = _slicedToArray(_base$sincos7, 2),
      sge = _base$sincos8[0],
      cge = _base$sincos8[1];

  var ra = Math.atan2(cge * sgl, cgl);
  var dec = Math.asin(sge * sgl);
  return new base.Coord(ra, dec);
};
...
M.trueVSOP87 = function (planet, jde) {
  ...
  var s = lon + Math.PI;
  // FK5 correction.
  var glp = base.horner(base.J2000Century(jde), s, -1.397 * Math.PI / 180, -0.00031 * Math.PI / 180);

  var _base$sincos9 = base.sincos(glp),
      _base$sincos10 = _slicedToArray(_base$sincos9, 2),
      sglp = _base$sincos10[0],
      cglp = _base$sincos10[1];

  var gDgb = 0.03916 / 3600 * Math.PI / 180 * (cglp - sglp);
  // (25.9) p. 166
...
M.apparentVSOP87 = function (planet, jde) {
  ...
  var gDgps = nutation.nutation(jde)[0];
...
M.apparentEquatorialVSOP87 = function (planet, jde) {
  ...
  var _toEquatorial = new coord.Ecliptic(gl, lat).toEquatorial(ge),
      ra = _toEquatorial.ra,
